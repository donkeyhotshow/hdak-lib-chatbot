...
| `OWNER_OPEN_ID` | Yes (prod) | Required in production by server startup checks |
| `OAUTH_SERVER_URL` | Usually yes | Required for OAuth flows |
| `VITE_APP_ID` | Usually yes | Required for OAuth flows |
| `VITE_OAUTH_PORTAL_URL` | Usually yes | URL shown for unauthenticated users |
| `DATABASE_URL` | Optional | MySQL URL (`mysql://user:password@host:3306/dbname`); if omitted app runs in mock mode |
