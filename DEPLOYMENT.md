...
- One LLM endpoint URL: `BUILT_IN_FORGE_API_URL` or `FORGE_API_URL`
- One LLM API key: `BUILT_IN_FORGE_API_KEY` or `FORGE_API_KEY` or `OPENAI_API_KEY`
- `OWNER_OPEN_ID` (required for production startup checks)
- Optional: external MySQL `DATABASE_URL` if you want persistent chat history
...
| Variable                                                                | Required to boot on Render (`NODE_ENV=production`) | Notes                                                                                  |
| ----------------------------------------------------------------------- | -------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `BUILT_IN_FORGE_API_URL` **or** `FORGE_API_URL`                         | Yes                                                | Base URL of your OpenAI-compatible endpoint                                            |
| `BUILT_IN_FORGE_API_KEY` **or** `FORGE_API_KEY` **or** `OPENAI_API_KEY` | Yes                                                | API key for the endpoint (mark as Secret)                                              |
| `JWT_SECRET`                                                            | Yes                                                | Auto-generated by Render (`generateValue: true`)                                       |
| `OWNER_OPEN_ID`                                                         | Yes                                                | Required in production by server startup checks                                        |
| `OAUTH_SERVER_URL`                                                      | No                                                 | Needed for OAuth sign-in flows                                                         |
| `VITE_APP_ID`                                                           | No                                                 | Needed for OAuth sign-in flows                                                         |
