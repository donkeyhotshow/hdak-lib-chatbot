...
export async function POST(request: NextRequest) {
  ...
  if (!endpoint || !keys?.p256dh || !keys?.auth) {
    return NextResponse.json({ error: 'Відсутні обовʼязкові поля: endpoint, keys.p256dh, keys.auth' }, { status: 400 });
  }
  ...
  if (remindAt && isNaN(remindAtDate!.getTime())) {
    return NextResponse.json({ error: 'Невалідна дата remindAt' }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: 'Помилка збереження підписки' }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Помилка видалення підписки' }, { status: 500 });
  }
}
