...
const markdownComponents = {
  ...
  tr: (props: React.HTMLAttributes<HTMLTableRowElement>) => (
    <tr
      className="border-b border-[#E5E1D8] odd:bg-white even:bg-[#FDFBF7] last:border-0"
      {...props}
    />
  ),
  th: (props: React.ThHTMLAttributes<HTMLTableCellElement>) => (
    <th
      className="text-left px-3 py-2 font-semibold text-[#1A1612] whitespace-nowrap"
      {...props}
    />
  ),
          ...
          >
            {QUICK_MENU.map((item, idx) => (
              ...
              >
                ...
                <span className="text-[14px] font-semibold text-[#2A2520]/80 group-hover:text-[#1A1612] flex-1">
                  {item.title}
                </span>
                <ChevronRight
                  size={16}
                  className="text-[#B87830]/30 group-hover:text-[#B87830]/60 shrink-0 transition-colors"
                />
              </motion.button>
            ))}
          </motion.div>
