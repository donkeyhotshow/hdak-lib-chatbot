...
const QUICK_MENU_UK = [
  {
    ...
    title: "Графік роботи",
    icon: Clock,
    kw: "Який графік роботи бібліотеки?",
  },
  {
    id: "register",
    title: "Як записатися",
    icon: ClipboardList,
    kw: "Як записатися до бібліотеки?",
  },
  {
    id: "catalog",
    title: "Каталог",
    icon: Search,
    kw: "Як знайти книгу в каталозі?",
  },
  {
    id: "contacts",
    title: "Контакти",
    icon: MapPin,
    kw: "Які контакти бібліотеки?",
  },
  {
    id: "rules",
    title: "Правила",
    icon: BookOpen,
    kw: "Які правила користування бібліотекою?",
  },
  {
    id: "resources",
    title: "Ресурси",
    icon: Globe,
    kw: "Які електронні ресурси доступні?",
  },
] as const;
...
const QUICK_MENU_EN = [
  {
    ...
    title: "Library hours",
    icon: Clock,
    kw: "What are the library hours?",
  },
  {
    id: "register",
    title: "How to register",
    icon: ClipboardList,
    kw: "How to register at the library?",
  },
  {
    id: "catalog",
    title: "Catalog",
    icon: Search,
    kw: "How to find a book in the catalog?",
  },
  {
    id: "contacts",
    title: "Contacts",
    icon: MapPin,
    kw: "What are the library contacts?",
  },
  {
    id: "rules",
    title: "Rules",
    icon: BookOpen,
    kw: "What are the library rules?",
  },
  {
    id: "resources",
    title: "Resources",
    icon: Globe,
    kw: "What electronic resources are available?",
  },
] as const;
...
const BookIcon = memo(function BookIcon({
  ...
  return (
    ...
    >
      <path
        d="M14 3C14 3 9 1.5 3 3.5V19C9 17 14 18.5 14 18.5"
        stroke="currentColor"
        strokeWidth="1.2"
        fill="none"
      />
      <path
        d="M14 3C14 3 19 1.5 25 3.5V19C19 17 14 18.5 14 18.5"
        stroke="currentColor"
        strokeWidth="1.2"
        fill="none"
        opacity="0.55"
  ...
  function MessageBubble({
    ...
    return (
      <motion.div
        ...
        className={cn(
          "flex gap-3 group animate-fade-in-slide-up",
      ...
      >
        ...
        <div
          className={cn(
            "flex flex-col",
            isUser ? "max-w-[85%] items-end" : "flex-1 min-w-0"
          )}
        >
          <div
            className={cn(
          ...
          >
            {isUser ? (
              ...
              <div className="prose prose-sm max-w-none prose-p:leading-[1.625] prose-strong:text-[#D4A853]">
                ...
                {isStreaming && (
                  <motion.span
                    className="inline-block w-[2px] h-[14px] bg-[#B87830] ml-[1px] align-baseline"
                    animate={{ opacity: [1, 0] }}
                    transition={{
                      duration: 0.5,
                      repeat: Infinity,
            ...
            {isError && (
              <div className="flex items-center gap-1.5 mt-2 text-red-500 text-[11px] font-semibold">
                ...
                {onRetry && (
                  <button
                    onClick={onRetry}
                    className="ml-1 underline decoration-red-300 underline-offset-2 hover:no-underline transition-all"
                  >
                    Повторити
                  </button>
                )}
          ...
          <div className="flex items-center gap-2 mt-1.5 px-2">
            <span className="text-[10px] text-[#7A756F]/40 tabular-nums font-medium uppercase tracking-wider">
...
const TypingIndicator = memo(function TypingIndicator() {
  return (
    <motion.div
      ...
      role="status"
      aria-label="Асистент думає"
    >
      <div className="w-8 h-8 rounded-full bg-[#B87830]/10 flex items-center justify-center shrink-0 mt-auto mb-1">
        <BookIcon size={14} className="text-[#B87830]" />
      </div>
      <div className="px-5 py-3.5 message-assistant rounded-[1.25rem] rounded-bl-[4px] bg-[#F9F7F2] border border-[#E5E1D8] shadow-sm">
        <div className="flex gap-1.5 items-center h-5">
          {[0, 1, 2].map(i => (
            <motion.div
              key={i}
              className="w-1.5 h-1.5 rounded-full bg-[#B87830]/40"
              animate={{ y: [0, -4, 0] }}
              transition={{
                duration: 0.6,
                repeat: Infinity,
...
function ChatAreaComponent({
  ...
  const getSuggestions = (content: string): string[] => {
    ...
    if (locale === "en") {
      if (/schedule|hours|working|open/i.test(c))
        return ["How to register?", "Library contacts"];
      if (/book|author|catalog|title|find/i.test(c))
        return ["Search more books", "New arrivals"];
      if (/research|thesis|scopus|repository|science/i.test(c))
        return ["Access Scopus?", "HDAK Repository"];
      if (/register|membership|card/i.test(c))
        return ["Library hours", "Contacts"];
    ...
    if (/контакт|телефон|email|адрес/i.test(c))
  ...
  const scrollToBottom = () => {
    try {
      ...
      logger.error("Error scrolling to bottom", error as Error);
  ...
  const { lastMsg, lastMsgHasCatalog, lastAssistantContent } = useMemo(() => {
    ...
    const hasCatalog = lastAssistantMsg
      ? lastAssistantMsg.content?.includes("[РЕЗУЛЬТАТИ КАТАЛОГУ") ||
        lastAssistantMsg.content?.includes("Знайдено за")
      : false;
    return {
      lastMsg: last,
      lastMsgHasCatalog: hasCatalog,
  ...
  if (messages.length === 0 && !isLoadingConversation) {
    return (
      <motion.div
        ...
        initial="initial"
        animate="animate"
        className="flex-1 overflow-y-auto custom-scrollbar chat-scroll-area flex flex-col items-center justify-center px-6 py-12"
      >
        <div className="w-full max-w-[560px] flex flex-col items-center">
          <motion.div
            variants={itemVariants}
            className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#B87830]/15 to-[#D4A853]/5 flex items-center justify-center mb-6 border border-[#B87830]/15 shadow-md shadow-[#B87830]/5 shrink-0"
          >
            <BookIcon size={24} className="text-[#B87830]" />
          </motion.div>
          <motion.h2
            variants={itemVariants}
            className="hero-title text-[28px] md:text-[32px] mb-2 text-center font-serif"
          >
            Чим я можу допомогти?
          </motion.h2>
          <motion.p
            variants={itemVariants}
            className="hero-subtitle text-[15px] mb-8 text-center text-[#7A756F]/80 max-w-[400px] leading-relaxed"
          >
            {locale === "en"
          ...
          {error && (
            <motion.div
              variants={itemVariants}
              className="mb-6 w-full p-4 bg-red-50 rounded-2xl text-red-600 text-[14px] border border-red-100/50 flex items-center justify-between gap-4 shadow-sm"
            >
              <span className="font-medium">{error}</span>
              {onRetry && (
                <button
                  onClick={onRetry}
                  className="shrink-0 text-[13px] font-bold underline underline-offset-4 hover:no-underline transition-all"
                >
                  Спробувати знову
                </button>
              )}
            ...
            {QUICK_MENU.map((item, idx) => (
              ...
              >
                <div className="w-10 h-10 rounded-xl bg-[#B87830]/5 flex items-center justify-center transition-colors group-hover:bg-[#B87830]/10">
                  <item.icon
                    size={18}
                    className="text-[#B87830]/70 group-hover:text-[#B87830]"
                  />
                </div>
                <span className="text-[14px] font-semibold text-[#2A2520]/80 group-hover:text-[#1A1612]">
                  {item.title}
                </span>
              </motion.button>
            ))}
          ...
          <motion.p
            variants={itemVariants}
            className="mt-10 text-[11px] font-bold text-[#7A756F]/40 uppercase tracking-[0.2em]"
          >
            ХДАК БІБЛІОТЕКА © 2024
          </motion.p>
        </div>
      </motion.div>
    );
        ...
        <div className="w-full max-w-[800px] mx-auto space-y-7">
          {error && (
            <div className="flex items-center justify-between gap-3 px-4 py-3 text-red-600 text-[13.5px] bg-red-50 border border-red-100 rounded-xl shadow-sm">
              ...
              {onRetry && (
                <button
                  onClick={onRetry}
                  className="shrink-0 text-[12px] font-bold underline underline-offset-4 hover:no-underline"
                >
                  Повторити
                </button>
              )}
          ...
          <AnimatePresence>
            {showChips && dynamicChips.length > 0 && (
              ...
              >
                <span className="text-[11px] font-bold text-[#7A756F]/40 uppercase tracking-wider block mb-3">
                  {locale === "en" ? "Quick suggestions:" : "Швидкі підказки:"}
                </span>
